...
import { controlEvidenceRequirementsFormSchema, ControlEvidenceRequirmentsFormValues } from "@/lib/schema/forms";
...
type Props = {
  ...
  action: (
    _prevState: any,
    params: FormData
  ) => Promise<FormState<ControlEvidenceRequirmentsFormValues>>;
};
...
export default function ControlEvidenceForm({ control, evidence, footer, action }: Props) {
  ...
  const form = useForm<ControlEvidenceRequirmentsFormValues>({
    resolver: zodResolver(controlEvidenceRequirementsFormSchema),
    defaultValues: {
