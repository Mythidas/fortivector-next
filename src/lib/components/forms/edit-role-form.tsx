...
import { editRoleFormSchema, EditRoleFormValues } from "@/lib/schema/forms";
...
export default function EditRoleForm({ role, action }: Props) {
  ...
  const form = useForm<EditRoleFormValues>({
    ...
    defaultValues: {
      ...
      user_access: role.access_rights?.users,
      role_access: role.access_rights?.roles,
      dashboard_access: role.access_rights?.dashboard,
      role_id: role.id
    }
  });
                ...
                <div key={category.id} className="space-y-2 pt-2">
                  {category.permissions.map((permission) => (
                    <div key={permission.id} className="flex items-center justify-between py-2 border-b border-border/20">
                      ...
                      <FormField
                        control={form.control}
                        name={permission.id as keyof EditRoleFormValues}
                        render={({ field }) => (
                          <FormItem>
                            <Select
